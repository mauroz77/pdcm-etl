...
def main(argv):
    ...
    search_index_patient_sample_df = transform_search_index_patient_sample(
        patient_sample_df, patient_df, sample_to_ontology_df, ontology_term_diagnosis_df
    )
...
def transform_search_index_patient_sample(
    ...
    sample_to_ontology_term_df = extend_sample_with_ontology_data(
        sample_to_ontology_df, ontology_term_diagnosis_df
    )

    cond = patient_sample_df.id == sample_to_ontology_term_df.sample_id
    patient_sample_ext_df = patient_sample_df.join(
        sample_to_ontology_term_df, on=cond, how="left"
    )
...
def get_formatted_patient(patient_df: DataFrame) -> DataFrame:
    ...
    patient_df = patient_df.withColumn(
        "patient_sex",
        when(
            lower(col("patient_sex")).contains("not"), Constants.NOT_PROVIDED_VALUE
        ).otherwise(lower(col("patient_sex"))),
    )
...
def extend_sample_with_ontology_data(
    sample_to_ontology_df: DataFrame, ontology_term_diagnosis_df: DataFrame
) -> DataFrame:
    ...
    sample_to_ontology_term_df = sample_to_ontology_term_df.withColumn(
        "search_terms",
        array_except(
            split(concat_ws("|", "term_name", "ancestors"), "\\|"),
            array(*map(lit, exclude_top_level_terms)),
        ),
    )
    sample_to_ontology_term_df = sample_to_ontology_term_df.withColumn(
        "cancer_system",
    ...
    sample_to_ontology_term_df = sample_to_ontology_term_df.withColumn(
        "cancer_system",
        when(col("cancer_system").isNull(), lit("Unclassified")).otherwise(
            col("cancer_system")
        ),
    )
